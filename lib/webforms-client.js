...
function describeLoginFailure(response) {
  const serverMessage = extractErrorMessage(response?.html);
  const details = serverMessage ? ` Servermeddelande: ${serverMessage}` : '';

  return `A-kod-inloggning misslyckades: login-sidan returnerades efter inloggningsförsöket. Uppdatera lösenordet i GitHub Secrets (AKOD_PASSWORD) och kontrollera att lösenordet inte har gått ut.${details}`;
}
...
class WebFormsClient {
  ...
  async login() {
    ...
    const loginResponse = await this.post(this.loginUrl, body, this.loginUrl);
    const servicePage = await this.get(this.serviceUrl);

    if (isLoginPage(servicePage.html)) {
      throw new Error(describeLoginFailure(loginResponse));
    }

    if (!hasAuthorisationForm(servicePage.html)) {
      throw new Error(
        'A-kod-inloggning lyckades, men ServiceBooking-formuläret kunde inte öppnas. Kontrollera om A-kod-tjänsten har ändrat sidstruktur.'
      );
    }
  ...
  async authorise(tank, ref, allowRelogin = true) {
    ...
    if (errorMessage) {
      if (/not yet ready for pick-up/i.test(errorMessage)) {
        return {
          success: false,
          status: 'Väntar',
          aKod: null,
